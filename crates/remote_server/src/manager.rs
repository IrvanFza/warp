...
impl RemoteServerManager {
    ...
    async fn run_connect_and_handshake(
        ...
        // Phase 3: Version compatibility check. If the daemon reports a
        // version that disagrees with the client's release tag, the
        // versioned path on disk is stale -- delete it so the next
        // reconnect reinstalls, and surface the mismatch the same way
        // as any other Initialize-phase failure so the existing
        // failed-banner path in the controller fires. Without this,
        // the session silently loses remote-server features and the
        // user has no signal to reconnect.
